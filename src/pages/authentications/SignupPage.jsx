...
export default function SignUpPage() {
    ...
    const validationSchema = Yup.object({
        fullName: Yup.string().required('Full name is required!').min(2, 'Min. 2 karakter'),
        username: Yup.string().required('Username is required!').min(4, 'Min. 4 karakter').matches(/^[a-zA-Z0-9_]+$/, 'Only letters, numbers, underscores'),
        email: Yup.string().email('Invalid email format').required('Email is required!'),
        password: Yup.string().required('Password is required!').min(6, 'Min. 6 karakter'),
    ...
    const handleSubmit = async (data) => {
        try {
            ...
            const result = await UserDAO.signUp({
                ...
                role: 'user',
                groups: [], // kosong dulu — akan diisi di onboarding
            });
            ...
            // Selalu ke onboarding untuk pilih grup
            localStorage.setItem('needsOnboarding', '1');
            message('Akun berhasil dibuat! Pilih grupmu dulu 🎉', 'success');
    ...
    return (
        <div style={{ minHeight: '100vh', background: '#f7f7fb', display: 'flex', flexDirection: 'column', alignItems: 'center', fontFamily: '"Nunito", sans-serif', overflowX: 'hidden' }}>
            <style>{`
                ...
                * { box-sizing: border-box; }
                ...
                .sp-input {
                    ...
                    border: 1.5px solid #e8e8f0; background: #fff; color: #1a1a2e;
                    font-size: 14px; font-family: 'Nunito', sans-serif; font-weight: 600;
                    outline: none; transition: all 0.2s; box-shadow: 0 2px 8px rgba(0,0,0,0.04);
                }
                .sp-input::placeholder { color: #b0b0c0; font-weight: 500; }
                .sp-input:focus { border-color: #F5B800; box-shadow: 0 0 0 3px rgba(245,184,0,0.15), 0 2px 8px rgba(0,0,0,0.06); }
                ...
                .sp-btn-main:hover:not(:disabled) { transform: translateY(-2px); box-shadow: 0 10px 28px rgba(245,184,0,0.5); background: #e6ac00; }
                .sp-btn-main:disabled { opacity: 0.6; cursor: default; }
                .sp-google-btn {
                    width: 100%; padding: 13px 16px; border-radius: 50px; border: 1.5px solid #e8e8f0;
                    background: #fff; color: #1a1a2e; font-size: 14px; font-weight: 700;
                    font-family: 'Nunito', sans-serif; cursor: pointer;
                    display: flex; align-items: center; justify-content: center; gap: 10px;
                    transition: all 0.2s; box-shadow: 0 2px 8px rgba(0,0,0,0.05);
                }
                .sp-google-btn:hover { transform: translateY(-2px); box-shadow: 0 6px 20px rgba(0,0,0,0.1); }
                .sp-divider { display: flex; align-items: center; gap: 12px; margin: 18px 0; }
                .sp-divider::before, .sp-divider::after { content: ''; flex: 1; height: 1.5px; background: #ececf4; border-radius: 2px; }
                .sp-divider span { font-size: 13px; color: #b0b0c0; font-weight: 700; }
                .sp-label { display: block; font-size: 12px; font-weight: 700; color: #8888a0; margin-bottom: 7px; text-transform: uppercase; letter-spacing: 0.08em; }
                .sp-err { font-size: 11.5px; color: #ff6b6b; margin-top: 4px; font-weight: 600; }
                .pw-wrap { position: relative; }
                .pw-toggle { position: absolute; right: 14px; top: 50%; transform: translateY(-50%); background: none; border: none; cursor: pointer; padding: 4px; color: #b0b0c0; display: flex; align-items: center; }
                .pw-toggle:hover { color: #7c6be0; }
            `}</style>

            {/* TOP HERO */}
            <div className="sp-hero" style={{
                width: '100%', background: 'linear-gradient(160deg, #6C63E0 0%, #8B80F8 60%, #a89cf8 100%)',
                borderRadius: '0 0 40px 40px', paddingTop: 40, paddingBottom: 8,
                ...
                <div style={{ animation: 'floatIllo 4s ease-in-out infinite' }}>
                    <div style={{ width: 130, height: 130, borderRadius: '50%', background: 'rgba(255,255,255,0.15)', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 8px 32px rgba(0,0,0,0.15)', marginBottom: 8 }}>
                        <div style={{ width: 112, height: 112, borderRadius: '50%', background: 'rgba(255,255,255,0.2)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                            <svg width="80" height="80" viewBox="0 0 104 104" fill="none">
                ...
                <div style={{ textAlign: 'center', paddingBottom: 20 }}>
                    <h1 style={{ fontFamily: '"Nunito",sans-serif', fontWeight: 900, fontSize: 26, color: '#fff', margin: '4px 0 4px', letterSpacing: '-0.02em' }}>Create Account</h1>
                    <p style={{ color: 'rgba(255,255,255,0.65)', fontSize: 13, fontWeight: 600, margin: 0 }}>Join and start your journey</p>
                </div>
            </div>

            {/* FORM */}
            <div className="sp-card" style={{ width: '100%', maxWidth: 420, padding: '24px 24px 0' }}>
                <form onSubmit={formik.handleSubmit}>
                    ...
                    <div style={{ marginBottom: 14 }}>
                        ...
                        <input className={`sp-input ${formik.touched.fullName && formik.errors.fullName ? 'has-error' : ''}`}
                            name="fullName" placeholder="John Doe"
                            onChange={formik.handleChange} onBlur={formik.handleBlur} value={formik.values.fullName} autoComplete="name" />
                        {formik.touched.fullName && formik.errors.fullName && <div className="sp-err">{formik.errors.fullName}</div>}
                    </div>

                    {/* Username */}
                    <div style={{ marginBottom: 14 }}>
                        <label className="sp-label">Username</label>
                        <input className={`sp-input ${formik.touched.username && formik.errors.username ? 'has-error' : ''}`}
                            name="username" placeholder="johndoe123"
                            onChange={formik.handleChange} onBlur={formik.handleBlur} value={formik.values.username} autoComplete="username" />
                        {formik.touched.username && formik.errors.username && <div className="sp-err">{formik.errors.username}</div>}
                    </div>

                    {/* Email */}
                    <div style={{ marginBottom: 14 }}>
                        <label className="sp-label">Email Address</label>
                        <input className={`sp-input ${formik.touched.email && formik.errors.email ? 'has-error' : ''}`}
                            name="email" type="email" placeholder="john@example.com"
                            onChange={formik.handleChange} onBlur={formik.handleBlur} value={formik.values.email} autoComplete="email" />
                        {formik.touched.email && formik.errors.email && <div className="sp-err">{formik.errors.email}</div>}
                    </div>
                    ...
                    <div style={{ marginBottom: 14 }}>
                        ...
                        <div className="pw-wrap">
                            <input className={`sp-input ${formik.touched.password && formik.errors.password ? 'has-error' : ''}`}
                        ...
                        {formik.touched.password && formik.errors.password && <div className="sp-err">{formik.errors.password}</div>}
                    </div>
                    ...
                    <div style={{ marginBottom: 24 }}>
                        ...
                        <div className="pw-wrap">
                            <input className={`sp-input ${formik.touched.confirmPassword && formik.errors.confirmPassword ? 'has-error' : ''}`}
                                name="confirmPassword" type={showConfirm ? 'text' : 'password'} placeholder="••••••••"
                        ...
                        {formik.touched.confirmPassword && formik.errors.confirmPassword && <div className="sp-err">{formik.errors.confirmPassword}</div>}
                    </div>
