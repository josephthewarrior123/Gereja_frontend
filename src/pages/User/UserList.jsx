...
function StatCard({ role, total, selected, onClick }) {
    ...
    return (
        <Box onClick={onClick} sx={{
            ...
            <Typography sx={{ fontSize: 34, fontWeight: 800, color: isSelected ? accent.from : '#0f172a', lineHeight: 1, fontFamily: '"Outfit", sans-serif', letterSpacing: '-0.03em', transition: 'color .18s' }}>{total}</Typography>
            <Typography sx={{ fontSize: 11, color: '#cbd5e1', mt: 0.5 }}>{role === 'ALL' ? 'total users' : 'registered'}</Typography>
        </Box>
    );
}
...
function RoleBadge({ role, onClick }) {
    ...
    return (
        <Box onClick={onClick} sx={{
            ...
            transition: 'opacity .15s', fontFamily: '"DM Sans", sans-serif', letterSpacing: '0.03em',
...
export default function UserListPage() {
    ...
    const [dataSourceOptions, setDataSourceOptions] = useState({ keyword: '', page: 0, limit: 10, total: 0, sortColumn: '', sortDirection: 'asc' });
    ...
    useEffect(() => { if (user) fetchUsers(); }, [user, dataSourceOptions.page, dataSourceOptions.limit, dataSourceOptions.sortColumn, dataSourceOptions.sortDirection, dataSourceOptions.keyword, selectedRole]);
    ...
    const handleSort = (columnKey) => setDataSourceOptions((prev) => ({ ...prev, sortColumn: columnKey, sortDirection: prev.sortColumn === columnKey ? (prev.sortDirection === 'asc' ? 'desc' : 'asc') : 'asc' }));
    ...
    const columns = [
        {
            ...
            render: (value, row) => (
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
                    <Avatar sx={{ width: 38, height: 38, bgcolor: roleColors[row.role] || '#64748B', fontSize: 14, fontWeight: 800, fontFamily: '"Outfit", sans-serif', boxShadow: `0 2px 8px ${roleColors[row.role] || '#64748B'}44` }}>
                        {value?.charAt(0)?.toUpperCase() || 'U'}
                    </Avatar>
                    <Box>
                        <Typography sx={{ fontSize: 14, fontWeight: 600, color: '#0f172a', fontFamily: '"Outfit", sans-serif' }}>{value}</Typography>
                        <Typography sx={{ fontSize: 11, color: '#94a3b8', fontFamily: '"DM Sans", sans-serif' }}>@{row.username}</Typography>
                    </Box>
                </Box>
            ),
        },
        { title: 'Email', dataIndex: 'email', key: 'email', sortable: false, render: (value) => <Typography sx={{ fontSize: 13, color: '#64748b', fontFamily: '"DM Sans", sans-serif' }}>{value}</Typography> },
        {
                        ...
                        {sortedSummaries.map((s) => {
                            ...
                            return (
                                <Box key={s.role} onClick={() => handleRoleFilterChange(s.role)} sx={{
                                    ...
                                    border: '1.5px solid', borderColor: selectedRole === s.role ? acc.from : '#e2e8f0',
                                    bgcolor: selectedRole === s.role ? acc.from : '#fff',
                                    color: selectedRole === s.role ? '#fff' : '#64748b', transition: 'all .15s',
                                }}>
                                ...
                                <CardContent sx={{ p: '18px !important' }}>
                                    <Box sx={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between' }}>
                                        <Stack direction="row" spacing={1.5} alignItems="center" sx={{ flex: 1, minWidth: 0, pr: 1 }}>
                                            ...
                                            <Box sx={{ minWidth: 0 }}>
                                                <Typography noWrap sx={{ fontSize: 15, fontWeight: 700, color: '#0f172a', fontFamily: '"Outfit", sans-serif', lineHeight: 1.2, mb: 0.5 }}>@{u.username}</Typography>
                                                <Stack direction="row" spacing={0.5} flexWrap="wrap" sx={{ rowGap: 0.5 }}>
                                                    {(u.role === 'user' ? u.groups : u.managedGroups).map((g) => (
                                                        <Box key={g} sx={{ px: 1, py: 0.2, borderRadius: '6px', bgcolor: '#f8fafc', border: '1px solid #e2e8f0', fontSize: 10, color: '#475569', fontWeight: 600, fontFamily: '"DM Sans", sans-serif' }}>{g}</Box>
                                                    ))}
                ...
                {total > 0 && (
                    <Box sx={{ mt: 3, pt: 3, borderTop: '1px solid #E2E8F0' }}>
                        <Stack direction="row" justifyContent="space-between" sx={{ mb: 2 }}>
                            <Typography sx={{ fontSize: 12, color: '#94a3b8', fontFamily: '"DM Sans", sans-serif' }}>{startIndex + 1}–{Math.min(startIndex + mobileLimit, total)} of {total}</Typography>
                    ...
                    {kebabUser && (
                        <Box sx={{ px: 2, pt: 2.5 }}>
                            ...
                            <Stack direction="row" spacing={1.5} alignItems="center" sx={{ mb: 2, px: 1 }}>
                                <Avatar sx={{ width: 40, height: 40, bgcolor: roleColors[kebabUser.role] || '#64748B', fontWeight: 800, fontFamily: '"Outfit", sans-serif' }}>
                                    {kebabUser.fullName?.charAt(0)?.toUpperCase() || 'U'}
                                </Avatar>
                                <Box>
                                    <Typography sx={{ fontSize: 14, fontWeight: 700, color: '#0f172a', fontFamily: '"Outfit", sans-serif' }}>{kebabUser.fullName}</Typography>
                                    <Typography sx={{ fontSize: 11, color: '#94a3b8' }}>@{kebabUser.username}</Typography>
                                </Box>
                            </Stack>
                            <Divider sx={{ mb: 1.5 }} />
                            <Stack spacing={0.5}>
                                <MenuItem onClick={() => { setKebabOpen(false); navigate(`/users/${kebabUser.username}/edit`); }} sx={{ borderRadius: '10px', py: 1.2 }}>
    ...
    const renderDesktopView = () => (
        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 3, p: 3, bgcolor: '#f8fafc', minHeight: '100%' }}>
            ...
            <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 2 }}>
                <Box>
                    <Typography sx={{ fontSize: 11, fontWeight: 700, letterSpacing: '0.12em', textTransform: 'uppercase', color: '#94a3b8', fontFamily: '"DM Sans", sans-serif', mb: 0.3 }}>Management</Typography>
            ...
            <Box sx={{ maxWidth: 380 }}>
                <TextField fullWidth placeholder="Search by name, username or email…" onKeyPress={(e) => { if (e.key === 'Enter') handleFilterChange('keyword', e.target.value); }}
                    InputProps={{ startAdornment: <InputAdornment position="start"><Icon icon="mdi:magnify" color="#94A3B8" width={18} /></InputAdornment>, sx: { borderRadius: '12px', bgcolor: '#fff', fontSize: 13, fontFamily: '"DM Sans", sans-serif', '& .MuiOutlinedInput-notchedOutline': { borderColor: '#E2E8F0' }, '&:hover .MuiOutlinedInput-notchedOutline': { borderColor: '#cbd5e1' }, '&.Mui-focused .MuiOutlinedInput-notchedOutline': { borderColor: '#6366f1', borderWidth: '1.5px' } } }} />
            </Box>
            {user?.role === 'super_admin' && (
                <Box sx={{ display: 'flex', gap: 2 }}>
                    {sortedSummaries.map((s) => <StatCard key={s.role} role={s.role} total={s.total} selected={selectedRole} onClick={() => handleRoleFilterChange(s.role)} />)}
                </Box>
            )}
            <Box sx={{ bgcolor: '#fff', borderRadius: '16px', border: '1px solid #f1f5f9', boxShadow: '0 1px 12px rgba(0,0,0,0.04)', overflow: 'hidden' }}>
                <CustomDatatable dataSource={dataSource} columns={columns} page={dataSourceOptions.page} limit={dataSourceOptions.limit} totalRecords={dataSourceOptions.total} handlePageChange={handlePageChange} handleLimitChange={handleLimitChange} handleSort={handleSort} sortColumn={dataSourceOptions.sortColumn} sortDirection={dataSourceOptions.sortDirection} />
            </Box>
        </Box>
    );
            ...
            <Dialog open={isDeleteDialogOpen} onClose={closeDeleteDialog} maxWidth="xs" fullWidth fullScreen={isMobile}
                ...
                <Box sx={{ p: 3 }}>
                    <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', mb: 2 }}>
                        <Box>
                            <Typography sx={{ fontSize: 17, fontWeight: 800, color: '#0f172a', fontFamily: '"Outfit", sans-serif', mb: 0.3 }}>Delete User</Typography>
                            <Typography sx={{ fontSize: 12, color: '#94a3b8', fontFamily: '"DM Sans", sans-serif' }}>This action cannot be undone</Typography>
                        </Box>
                        <IconButton onClick={closeDeleteDialog} size="small" sx={{ borderRadius: '8px', color: '#94a3b8' }}><Icon icon="mdi:close" width={18} /></IconButton>
                    </Box>
                        ...
                        <Button variant="contained" color="error" startIcon={<Icon icon="mdi:delete" />}
                            ...
                            onClick={async () => {
                                try {
                                    ...
                                    closeDeleteDialog(); fetchUsers();
                                } catch (e) { message(e.message || 'Gagal menghapus user', 'error'); }
                                finally { loading.stop(); }
                            }}>
                            Hapus User
                        </Button>
