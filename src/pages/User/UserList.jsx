...
export default function UserListPage() {
    ...
    const fetchUsers = async () => {
        try {
            ...
            if (user?.role === 'super_admin') {
                // super_admin: pakai endpoint lama, lihat semua user
                ...
                // admin: pakai AdminDAO, otomatis filtered by managedGroups di backend
    ...
    useEffect(() => {
        if (allUsers.length > 0) applyFilters(allUsers, selectedRole, dataSourceOptions);
    ...
    const handleSort = (columnKey) => {
        setDataSourceOptions((prev) => ({
            ...
            sortDirection: prev.sortColumn === columnKey
    ...
    const columns = [
        {
            ...
            render: (value, row) => (
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
                    <Avatar sx={{ width: 36, height: 36, bgcolor: roleColors[row.role] || '#64748B', fontSize: 14, fontWeight: 700 }}>
        ...
        },
        {
            ...
            render: (value) => <Typography variant="body2" color="text.secondary">{value}</Typography>,
        },
        {
            ...
            render: (value, row) => {
                ...
                return (
                    <Stack direction="row" spacing={0.5} flexWrap="wrap">
                        {list.map((g) => <Chip key={g} label={g} size="small" variant="outlined" sx={{ fontSize: 11, height: 22 }} />)}
                    </Stack>
                );
            },
        },
        {
            ...
            render: (value, row) => (
                <Chip
                    ...
                    sx={{ fontWeight: 700, fontSize: 11, bgcolor: roleColors[value] || '#64748B', color: '#fff', cursor: 'pointer', '&:hover': { opacity: 0.85 } }}
                />
            ),
        },
        {
            title: 'Joined', dataIndex: 'createdAt', key: 'createdAt', sortable: false,
            render: (value) => <Typography variant="body2" color="text.secondary">{formatDate(value)}</Typography>,
        },
        {
        ...
        return (
            <Box sx={{ p: 2, bgcolor: '#F8FAFC', minHeight: '100%' }}>
                <Stack direction="row" spacing={1} sx={{ mb: 2 }}>
                    <Button fullWidth variant="contained" startIcon={<Icon icon="mdi:account-plus-outline" />}
                        onClick={() => navigate('/users/create')} sx={{ textTransform: 'none', borderRadius: '12px' }}>
                        Create User
                    </Button>
                    {user?.role === 'super_admin' && (
                        <Button fullWidth variant="outlined" startIcon={<Icon icon="mdi:shield-plus-outline" />}
                            onClick={() => navigate('/users/create-admin')} sx={{ textTransform: 'none', borderRadius: '12px' }}>
                            Create Admin
                        </Button>
                    )}
                </Stack>
                ...
                {user?.role === 'super_admin' && (
                    <Box sx={{ display: 'flex', gap: 1, overflowX: 'auto', pb: 2, mb: 1, '&::-webkit-scrollbar': { display: 'none' } }}>
                        {sortedSummaries.map((s) => (
                            <Chip key={s.role} label={`${roleLabels[s.role]} (${s.total})`}
                                onClick={() => handleRoleFilterChange(s.role)}
                                sx={{
                                    border: '1px solid', borderColor: selectedRole === s.role ? '#1E3A8A' : '#E2E8F0',
                ...
                {paginated.length === 0 ? (
                    <Box sx={{ textAlign: 'center', py: 8 }}>
                        ...
                        <Typography variant="body1" sx={{ mt: 2, color: '#94A3B8', fontWeight: 500 }}>No users found</Typography>
                    </Box>
                ) : (
                    <Stack spacing={2}>
                        {paginated.map((u) => (
                            <Card key={u.id} sx={{ borderRadius: '24px', boxShadow: '0 4px 12px rgba(0,0,0,0.03)', border: '1px solid #F1F5F9' }}>
                                <CardContent sx={{ p: '20px !important' }}>
                                    <Stack direction="row" spacing={2} alignItems="center" sx={{ mb: 2 }}>
                                        <Avatar sx={{ width: 48, height: 48, bgcolor: roleColors[u.role] || '#64748B', fontWeight: 700 }}>
                                            {u.fullName?.charAt(0)?.toUpperCase() || 'U'}
                                        </Avatar>
                                        <Box sx={{ flex: 1 }}>
                                            <Typography variant="subtitle1" fontWeight={700} color="#1E293B">{u.fullName}</Typography>
                                        ...
                                        <Chip label={roleLabels[u.role] || u.role} size="small"
                                            ...
                                            sx={{ fontWeight: 700, fontSize: '0.65rem', bgcolor: roleColors[u.role] || '#64748B', color: '#fff', borderRadius: '8px', cursor: 'pointer' }}
                                        />
                                    </Stack>
                                    ...
                                    <Grid container spacing={2} sx={{ mb: 2 }}>
                                        <Grid item xs={6}>
                                            ...
                                            <Typography variant="body2" fontWeight={600} color="#334155" sx={{ mt: 0.5, wordBreak: 'break-all' }}>{u.email}</Typography>
                                        </Grid>
                                        <Grid item xs={6}>
                                            <Typography variant="caption" sx={{ color: '#94A3B8', fontWeight: 700, textTransform: 'uppercase' }}>Groups</Typography>
                                            <Box sx={{ mt: 0.5 }}>
                                                {(u.role === 'user' ? u.groups : u.managedGroups).length > 0 ? (
                                                    <Stack direction="row" spacing={0.5} flexWrap="wrap">
                                                        {(u.role === 'user' ? u.groups : u.managedGroups).map((g) => (
                                                            <Chip key={g} label={g} size="small" variant="outlined" sx={{ fontSize: 10, height: 20 }} />
                                                        ))}
                                                    </Stack>
                                                ) : (
                                        ...
                                        <Grid item xs={6}>
                                            ...
                                            <Typography variant="body2" fontWeight={600} color="#334155" sx={{ mt: 0.5 }}>{formatDate(u.createdAt)}</Typography>
                                        </Grid>
                                    </Grid>
                                    <Stack direction="row" spacing={1} justifyContent="flex-end" sx={{ pt: 1, borderTop: '1px solid #F8FAFC' }}>
                                        <IconButton size="small" onClick={() => navigate(`/users/${u.username}/edit`)}
                                            sx={{ color: '#64748B' }} title="Edit user">
                ...
                {total > 0 && (
                    <Box sx={{ mt: 4, pt: 3, borderTop: '1px solid #E2E8F0', pb: 4 }}>
                        ...
                        <Stack direction="row" spacing={2}>
                            ...
                            <Button fullWidth variant="outlined" disabled={endIndex >= total}
                                onClick={() => setDataSourceOptions((prev) => ({ ...prev, page: prev.page + 1 }))}
    ...
    const renderDesktopView = () => (
        <CustomColumn className="gap-y-8 max-h-full">
            <CustomRow className="gap-x-3">
                <Button variant="contained" startIcon={<Icon icon="mdi:account-plus-outline" />}
                    onClick={() => navigate('/users/create')} sx={{ textTransform: 'none' }}>
                    Create User
                </Button>
                {user?.role === 'super_admin' && (
                    <Button variant="outlined" startIcon={<Icon icon="mdi:shield-plus-outline" />}
                        onClick={() => navigate('/users/create-admin')} sx={{ textTransform: 'none' }}>
                        Create Admin
                    </Button>
                )}
            </CustomRow>
            ...
            <CustomRow className="gap-x-4">
                <CustomTextInput placeholder="Search users..." searchIcon
            ...
            {user?.role === 'super_admin' && (
                <CustomRow className="lg:gap-x-6 md:gap-x-2 sm:gap-x-0 items-start">
                    {sortedSummaries.map((s) => (
                        <div key={s.role} onClick={() => handleRoleFilterChange(s.role)}
                            className={`cursor-pointer rounded-lg transition-all duration-200 ${selectedRole === s.role ? 'border-2 border-blue-500' : 'border border-transparent'}`}
                            style={{ width: '100%', height: '100%', display: 'flex' }}>
                            <CustomDashboardStatsCard value={s.total} label={roleLabels[s.role] || s.role} className="w-full h-full" />
                        </div>
                    ))}
                </CustomRow>
            )}
        ...
        <>
            ...
            <Menu anchorEl={roleMenuAnchor} open={Boolean(roleMenuAnchor)} onClose={handleRoleMenuClose}
                ...
                <MenuItem onClick={() => handleRoleUpdate('user')} disabled={updatingRole}>
                    <ListItemIcon><Icon icon="mdi:account-outline" color={roleColors.user} width={20} /></ListItemIcon>
                    <ListItemText primary="User" />
                </MenuItem>
                <MenuItem onClick={() => handleRoleUpdate('admin')} disabled={updatingRole}>
                    <ListItemIcon><Icon icon="mdi:shield-outline" color={roleColors.admin} width={20} /></ListItemIcon>
                    <ListItemText primary="Admin" />
                </MenuItem>
                {user?.role === 'super_admin' && (
                    <MenuItem onClick={() => handleRoleUpdate('super_admin')} disabled={updatingRole}>
                        <ListItemIcon><Icon icon="mdi:shield-crown-outline" color={roleColors.super_admin} width={20} /></ListItemIcon>
            ...
            <Dialog open={isDeleteDialogOpen} onClose={closeDeleteDialog} maxWidth="xs" fullWidth fullScreen={isMobile}>
                <Box sx={{ p: { xs: 2, sm: 3 } }}>
                    <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2 }}>
                        ...
                        <IconButton onClick={closeDeleteDialog} size="small"><Icon icon="mdi:close" /></IconButton>
                    </Box>
                    ...
                    <Box sx={{ display: 'flex', justifyContent: 'flex-end', gap: 1, flexDirection: { xs: 'column', sm: 'row' } }}>
                        <Button variant="outlined" onClick={closeDeleteDialog} fullWidth={isMobile}>Batal</Button>
                        <Button variant="contained" color="error" startIcon={<Icon icon="mdi:delete" />} fullWidth={isMobile}
                            onClick={async () => {
                                try {
                                    loading.start();
